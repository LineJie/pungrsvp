...
export default async (req: Request) => {
  ...
  if (req.method === "POST") {
    ...
    const { customerName, customerContact, date, time, tableId, tableName, floor, duration, notes, status, checkinAt } = body;
    ...
    // yang sama di jam yang sama tidak bisa di-double-book.
    {
      const existing = await db.select().from(bookings).where(eq(bookings.date, date));
  ...
  if (req.method === "PATCH") {
    ...
    if (isReschedule && existing) {
      ...
      const sameSlot = await db.select().from(bookings).where(eq(bookings.date, newDate));
