...
import { verifyPassword, ensureStaffTable, ensureLocationColumns } from "../../db/authUtils.js";
...
export default async (req: Request) => {
      ...
      if (username === SUPERADMIN_USERNAME && superadminPw && password === superadminPw) {
              return Response.json({ role: "superadmin", name: "Super Admin", location: "all" });
      }
      ...
      return Response.json({ role: rows[0].role, name: rows[0].name, location: rows[0].location || "surabaya" });
};
